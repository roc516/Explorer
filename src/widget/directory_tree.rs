...
use explorer_core::{load_tree_children, DirectoryTree, TreeRow};
...
impl DirectoryTreeWidget {
    pub fn new() -> Self {
        Self {
            state: DirectoryTree::new(),
            width: Length::Fixed(NAV_PANE_WIDTH),
        }
    }

    pub fn update(&mut self, message: tree::Message) -> (Task<tree::Message>, Option<Action>) {
        match message {
            ...
            tree::Message::ChildrenLoaded(path, result) => {
    ...
    pub fn sync_path(&mut self, path: &Path) -> Task<tree::Message> {
        let pending = self.state.sync_selection(path);
        Task::batch(pending.into_iter().map(load_children_task))
    }

    pub fn view(&self, bundle: explorer_core::LanguageBundle) -> Element<'_, tree::Message> {
        let rows = self.state.rows();
        let no_locations = bundle.tr(explorer_core::ids::TREE_NO_LOCATIONS);
        let content: Element<'_, tree::Message> = if rows.is_empty() {
...
fn load_children_task(path: PathBuf) -> Task<tree::Message> {
    Task::perform(
        {
            let load_path = path.clone();
            async move { load_tree_children(&load_path) }
        },
        move |result| tree::Message::ChildrenLoaded(path, result),
    )
}

fn view_row(row: TreeRow) -> Element<'static, tree::Message> {
    let chevron = chevron_widget(&row);
    let folder = tree_icons::folder::<tree::Message>(
    ...
    .on_press(tree::Message::Select(row.path.clone()));
...
fn chevron_widget(row: &TreeRow) -> Element<'static, tree::Message> {
    if !row.expandable {
        return Space::new()
    ...
    button(
        tree_icons::chevron::<tree::Message>(
            row.expanded,
            row.loading,
            CHEVRON_WIDTH,
            HEIGHT_LIST_ROW,
    ...
    .on_press(tree::Message::Toggle(row.path.clone()))
    .width(Length::Fixed(CHEVRON_WIDTH))
    .height(Length::Fixed(HEIGHT_LIST_ROW))
    .padding(0)
